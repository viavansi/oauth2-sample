...
public class OAuthUtils {
	...
	public static String getAccessToken(OAuth2Details oauthDetails) {
		...
		parametersBody.add(new BasicNameValuePair(
					OAuthConstants.PASSWORD, oauthDetails.getPassword()));
		...
		try {
			...
			if (code == OAuthConstants.HTTP_UNAUTHORIZED) {
				...
				code = response.getStatusLine().getStatusCode();
				if (code == 401 || code == 403) {
					System.out.println("Could not authenticate using client credentials.:"+code);
					throw new RuntimeException(
	...
	public static String postFile(OAuth2Details oauthDetails,String key,byte[] fileData) {
		String resourceURL = oauthDetails.getResourceServerUrl();
		
		HttpPost post = new HttpPost(resourceURL);
		String header=getAuthorizationHeaderForAccessToken(oauthDetails
				.getAccessToken());
		...
		ContentBody cd = new InputStreamBody(new ByteArrayInputStream(fileData), "file.txt");
		builder.addPart("documentKey",new StringBody(key,ContentType.TEXT_PLAIN));
		...
		try {
			...
			return handleResponse(response).get("fileKey").toString();
	...
	public static byte[] getFile(OAuth2Details oauthDetails) {
